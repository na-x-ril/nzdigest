...
export async function summarizeTranscriptGemini(input: SummarizeTranscriptGeminiInput): Promise<SummarizeTranscriptOutput> {
  console.log("Gemini summarizeTranscript called with input:", input);
  try {
    const result = await geminiSummarizeFlow(input);
     if (!result || typeof result !== 'object' || !result.topikUtama) {
        console.error("Gemini flow returned invalid or empty result:", result);
        throw new Error('Failed to generate summary content from Gemini AI or unexpected format.');
    }
    console.log("Gemini flow successfully processed output:", result);
    return result;
  } catch (error: any) {
    console.error("Error in Gemini summarizeTranscript:", error);
...
Mohon berikan ringkasan dalam format JSON di atas. Jangan awali respons Anda dengan frasa seperti "Berikut adalah ringkasan...". Langsung ke objek JSON. Berikan contoh spesifik dari transkrip jika relevan untuk memperjelas poin.`,
  config: {
    model: 'googleai/gemini-1.5-flash-latest', // Explicitly use the fully qualified model name
    temperature: 0.3,
    topP: 0.8,
...
const geminiSummarizeFlow = ai.defineFlow(
  ...
  async (input) => {
    console.log("Gemini summarize flow started with input:", input);
    try {
      const { output } = await geminiPrompt(input);
      if (!output) {
        console.error("Gemini prompt returned no output.");
        throw new Error('Gemini prompt returned no output.');
      }
      console.log("Output from Gemini prompt:", output);
      return output;
    } catch (error: any) {
      console.error('Error during Gemini prompt execution:', error.message, error.stack);
